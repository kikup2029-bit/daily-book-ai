import { readableDate, tidyLabel, signedAmount, sharingLabel, nextShareMode } from "../src/lib/entry-format.ts";
...
ok(/2025/.test(readableDate("2025-07-31", today)), "…but a previous year keeps it, or it's ambiguous");
ok(!/^\d{4}-/.test(readableDate("2026-07-31", today)), "never renders the raw ISO form");
eq(readableDate("not-a-date", today), "not-a-date", "unparseable input passes through rather than showing Invalid Date");
...
eq(tidyLabel("iPhone case"), "IPhone case", "only the first letter is touched — no title-casing the whole string");
...
eq(signedAmount({ amount_in: 0, amount_out: 42.5 }), -42.5, "spending is negative, so the sign carries it");
...
eq(nextShareMode({ household_id: null, is_split: false }).mode, "visible", "private offers sharing");
eq(nextShareMode({ household_id: "h", is_split: false }).mode, "split", "shared offers splitting");
eq(nextShareMode({ household_id: "h", is_split: true }).mode, "private", "split offers going private");
ok(nextShareMode({ household_id: null, is_split: false }).label.length > 0, "every state has a written label");
