...
export function OfflineBar({ userId }: { userId: string | null | undefined }) {
  ...
  return (
    <div className="border-b bg-background/95 backdrop-blur">
      <div className="mx-auto w-full max-w-4xl px-5 py-2.5">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm">
          ...
          {online && waiting > 0 ? (
            <>
              ...
              <span className="font-semibold">
                {syncing ? "Sending" : "Waiting to send"} {waiting}{" "}
          ...
          {!online && waiting > 0 ? (
            <span className="text-muted-foreground">
              · {waiting} {waiting === 1 ? "entry" : "entries"} waiting
            </span>
          ) : null}
          ...
          {stuck > 0 ? (
            <span className="flex items-center gap-1.5 text-danger">
              ...
              <span className="font-semibold">
                {stuck} {stuck === 1 ? "entry" : "entries"} wouldn&apos;t save
              </span>
            </span>
          ) : null}
          ...
          <div className="mt-3 space-y-2 border-t pt-3">
            {waitingItems.map((item) => (
              <div key={item.id} className="flex items-start gap-3 text-xs">
                <span className="flex-1 text-muted-foreground">{describe(item)}</span>
            ...
            {stuckItems.length > 0 ? (
              <div className="mt-3 border-l-2 border-danger pl-3">
                <p className="text-xs text-danger">
                  These were refused {stuckItems[0].attempts} times. Usually that means the app was
                  updated or you were signed out — try again, and only discard one if you&apos;ve
                  already entered it another way.
                </p>
                {stuckItems.map((item) => (
                  <div key={item.id} className="mt-2 flex items-start gap-3 text-xs">
